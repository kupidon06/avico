...
def initiate_payment(request, order_id):
    ...
    try:
        ...
        # Remplacez ces chaînes par les noms de vos vues Django
        return_url = request.build_absolute_uri(reverse('ecommerce'))
        notify_url = request.build_absolute_uri(reverse('webhook'))

        response = cinetpay.initiate_payment(
            amount=order.debt_amount,
            transaction_id=transaction_id,
            description=f"Paiement pour la commande {order.id}",
            return_url=return_url,  # URL de retour après paiement
            notify_url=notify_url,  # Webhook pour confirmation
        )
