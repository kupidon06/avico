...
from apps.orders.models import BatchAllocation,OrderItem
...
def daily_log_list(request, batch_id):
    ...
    if from_date:
        ...
        daily_logs = daily_logs.filter(log_date__gte=from_date)

    if to_date:
        to_date = datetime.strptime(to_date, '%Y-%m-%d').date()
        daily_logs = daily_logs.filter(log_date__lte=to_date)
...
def feeding_list(request, batch_id):
    ...
    if from_date:
        from_date = datetime.strptime(from_date, '%Y-%m-%d').date()
        feedings = feedings.filter(feeding_date__gte=from_date)

    if to_date:
        to_date = datetime.strptime(to_date, '%Y-%m-%d').date()
        feedings = feedings.filter(feeding_date__lte=to_date)
...
def feeding_list_export(request, batch_id):
    ...
    for feeding in feedings:
        ws.append([feeding.feeding_date, feeding.batch.name, feeding.quantity, feeding.feed_type.name, feeding.feed_quantity_before, feeding.feed_quantity_after, feeding.details])
...
def egg_collection_list(request, batch_id):
    ...
    if from_date:
        from_date = datetime.strptime(from_date, '%Y-%m-%d').date()
        egg_collections = egg_collections.filter(collection_date__gte=from_date)

    if to_date:
        to_date = datetime.strptime(to_date, '%Y-%m-%d').date()
        egg_collections = egg_collections.filter(collection_date__lte=to_date)
    ...
    for egg_collection in egg_collections:
        if egg_collection.collection_date:
            ...
            egg_collection.collection_date = collection_date_str
...
def egg_collection_list_export(request, batch_id):
    ...
    for egg_collection in egg_collections:
        ws.append([egg_collection.collection_date, egg_collection.batch.name, egg_collection.quantity, egg_collection.cracked, egg_collection.details])
...
def treatment_history_list(request, batch_id):
    ...
    if from_date:
        from_date = datetime.strptime(from_date, '%Y-%m-%d').date()
        treatment_histories = treatment_histories.filter(treatment_date__gte=from_date)

    if to_date:
        to_date = datetime.strptime(to_date, '%Y-%m-%d').date()
        treatment_histories = treatment_histories.filter(treatment_date__lte=to_date)
