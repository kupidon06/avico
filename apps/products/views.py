...
from .models import Category, Product, ProductUnit, Discount, Media
...
def category_list(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            for file in files:
                media = Media(media_type='image', file=file, content_object=category)
                media.save()
...
def update_category(request, id):
    ...
    return render(request, 'apps/products/update_category.html', context)
...
def product_list(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            files = request.FILES.getlist('medias')
            for file in files:
                media = Media(media_type='image', file=file, content_object=product)
                media.save()
...
def update_product(request, id):
    ...
    return render(request, 'apps/products/update_product.html', context)
...
def product_unit_list(request):
    ...
    paginator = Paginator(product_units, 10)
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            product_unit = form.save()
            files = request.FILES.getlist('medias')
            for file in files:
                media = Media(media_type='image', file=file, content_object=product_unit)
                media.save()
...
def update_product_unit(request, id):
    ...
    return render(request, 'apps/products/update_product_unit.html', context)
